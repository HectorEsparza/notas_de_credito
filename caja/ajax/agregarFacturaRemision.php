<?php
  ...
  if($flag==0){
    ...
    $consulta = "SELECT NUMERO_ENTRADA FROM CARGAS WHERE ENTRADA=? ORDER BY NUMERO_ENTRADA DESC LIMIT 1";
    $resultado = $base->prepare($consulta);
    $resultado->execute(array($folio));
    $registro = $resultado->fetch(PDO::FETCH_NUM);
    $numeroEntrada = $registro[0];
    $resultado->closeCursor();
    ...
    for($i=1; $i <= $contador ; $i++){
      ...
      $resultado = $base->prepare($consulta);
      $resultado->execute(array($metodos[$i], $observaciones[$i], $folio, $fecha, $numeroEntrada, $facturas[$i]));
      $numeroEntrada++;
    }
    $resultado->closeCursor();
    //Consulta para obtener el total anterior de la cobranza
    $consulta = "SELECT TOTAL FROM CAJA WHERE CLAVE=?";
    $resultado = $base->prepare($consulta);
    $resultado->execute(array($folio));
    $registro = $resultado->fetch(PDO::FETCH_NUM);
    ...
    $consulta = "UPDATE CAJA SET TOTAL=? WHERE CLAVE=?";
    $resultado = $base->prepare($consulta);
