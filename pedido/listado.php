...
$query = $con->prepare("SELECT FOLIOINTERNO, NOCLIENTE, CLIENTE, CARTAFACTURA, TOTAL, FECHA, STATUS, USUARIO FROM PEDIDOS_VIS ORDER BY REGISTRO DESC LIMIT ? OFFSET ?");
$query->bind_param("ii",$limit,$offset);
$query->execute();

// vincular variables a la sentencia preparada
//$query->bind_result($id_usuario, $nombres,$apellidos);
$query->bind_result($folio, $nocliente, $cliente, $factura, $total, $fecha, $status,$usuario);
...
while ($query->fetch()) {
	...
	$data_json["cartafactura"] = $factura;
...
$cantidad_consulta = $con->query("select count(*) as total from PEDIDOS_VIS");
