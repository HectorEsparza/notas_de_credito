	...
	<? else :?>
			<header class="row">
				...
				<div class="container col-md-4">
					...
					<input type="button" style="float: right;" class="btn btn-primary" onclick="nota()" value='Nuevo Pedido' />
				</div>
			</header>
			<section>
      ...
      function nota(){
          ...
          setTimeout("location.href='visualizarPedidos.php'",500);
			...
			function saludo(folio){
					setTimeout("location.href='impresionPedido.php?folio="+folio+"'");
			}
			function sae(folio){
					setTimeout("location.href='capturaSae.php?folio="+folio+"'");
			}
			function prueba(folio){
					setTimeout("location.href='../cartaFactura/prueba.php?folio="+folio+"'");
			}
