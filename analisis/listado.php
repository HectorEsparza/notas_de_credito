...
$query = $con->prepare("SELECT CLAVEDEARTÍCULO, PRECIO, ID_VAZLO, PRECIO_VAZLO FROM PRODUCTOS1 LIMIT ? OFFSET ?");
$query->bind_param("ii",$limit,$offset);
$query->execute();

// vincular variables a la sentencia preparada
//$query->bind_result($id_usuario, $nombres,$apellidos);
$query->bind_result($id_apa, $precio_apa, $id_vazlo, $precio_vazlo);

// obtener valores
while ($query->fetch()) {
	$data_json = array();

	$data_json["id_apa"] = $id_apa;
	$data_json["precio_apa"] = $precio_apa;
	$data_json["id_vazlo"] = $id_vazlo;
	$data_json["precio_vazlo"] = $precio_vazlo;

	$data[]=$data_json;
}

// obtiene la cantidad de registros
$cantidad_consulta = $con->query("select count(*) as total from PRODUCTOS1");
