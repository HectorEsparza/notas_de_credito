...
$query = $con->prepare("SELECT FOLIOINTERNO, NOCLIENTE, CLIENTE, NOTASAE, TOTAL, FECHA, PDF, STATUS, RECEPCION FROM NOTAS_VIS ORDER BY REGISTRO DESC LIMIT ? OFFSET ?");
$query->bind_param("ii",$limit,$offset);
$query->execute();

// vincular variables a la sentencia preparada
//$query->bind_result($id_usuario, $nombres,$apellidos);
$query->bind_result($folio, $nocliente, $cliente, $nota, $total, $fecha, $pdf, $status, $recepcion);
...
while ($query->fetch()) {
  ...
  $data_json["fecha"] = $fecha;
	...
	$data_json["recepcion"] = $recepcion;

	$data[]=$data_json;
}

// obtiene la cantidad de registros
$cantidad_consulta = $con->query("select count(*) as total from NOTAS_VIS");
