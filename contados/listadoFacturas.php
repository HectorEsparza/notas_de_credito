...
$query = $con->prepare("SELECT CLAVE, CLIENTE, LEFT(NOMBRE, 15), ESTATUS, CARGAS.FECHA, DESCUENTO, IMPORTE, VENDEDOR,
												CONTADO.Folio, CONTADO.Fecha FROM CARGAS LEFT JOIN CONTADO ON CARGAS.idContado=CONTADO.idContado
												WHERE CARGAS.FECHA BETWEEN '2020-01-01' AND CURDATE() ORDER BY CARGAS.FECHA DESC LIMIT ? OFFSET ?");
$query->bind_param("ii",$limit,$offset);
$query->execute();

// vincular variables a la sentencia preparada
//$query->bind_result($id_usuario, $nombres,$apellidos);
$query->bind_result($clave, $cliente, $nombre, $estatus, $fecha, $descuento, $importe, $vendedor, $entrada, $fechaCorte);
